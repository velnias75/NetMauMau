...
NetMauMau::Common::base64_decode(std::string const &encoded_string) {
	...
	while(in_len-- && (encoded_string[in_] != '=') &&
			is_base64(static_cast<NetMauMau::Common::BYTE>(encoded_string[in_]))) {

		char_array_4[i++] = static_cast<NetMauMau::Common::BYTE>(encoded_string[in_]);
		++in_;

		if(i == 4) {

			for(i = 0; i < 4; ++i) char_array_4[i] =
					base64_chars.find(static_cast<std::string::value_type>(char_array_4[i]));
	...
	if(i) {
		...
		for(j = 0; j < 4; ++j) char_array_4[j] =
				base64_chars.find(static_cast<std::string::value_type>(char_array_4[j]));
